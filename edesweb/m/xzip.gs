...
rename( "edes/web/edesweb/_d_/cfg/eedd.lp", "edes/web/edesweb/_d_/cfg/ed.lp" );
Pie( false, $error );
exit;
}
...
$NoCopy = array( 'enc', 'bak', 'alb', 'hta', 'xar', 'old', 'OLD', 'no', 'NO' );
if( $VerLOG == 1 ){
...
if( isset($ALMACEN) ){
if( $TIPO_VERSION!='F' ) $dir_destino .= '/edesweb';
...
if( $TIPO_VERSION!='F' ){
$SubDir = true;
if( !is_dir($dir_destino.'/edesweb') ) mkdir($dir_destino.'/edesweb', 0777);
if( strtoupper(substr(PHP_OS,0,3))=='WIN' ){
$dir_destino .= '\\edesweb';
}else{
$dir_destino .= '/edesweb';
...
$dir_destino = str_replace(chr(92), "/", $dir_destino);
file_put_contents("edesweb/version", date("ymd"));
CopyDirectorio("{$dir_origen}", "{$dir_destino}", $Copy, $NoCopy);
...
$error = BackupFuentes('z', 'edesweb', false);
Pie(true, $error);
exit;
...
unlink($dorg.'/'.$file);
continue;
}else if( substr($file, -4)==".___" ){
unlink($dorg.'/'.$file);
continue;
}else if( substr_count($file, " - copia.") ){
unlink($dorg.'/'.$file);
continue;
}
if( file_exists($dorg.'/'.$file) != 1 ) die("<BR> >>>>>>>>>>>> No existe el fichero [".$file."]");
if( is_dir($dorg.'/'.$file) && $file!=$ddest ){
if( ($file[0]!='_' && substr($file,-1)!='_') || substr_count($dorg, "web/aplication") ){
...
global $_VERSION_, $_HD_MOTOR_, $_ENCODER_, $_ENCODERIONCUBE_, $_VERSIONZIP_, $_OK_ENCODER, $SERVER_SOFTWARE, $Dir_, $_DirBase;
if( $ConTH ) echo "<TR><TH colspan=2 id=c class=GRUPO>ZIP de fuentes Comprimidos</TH>";
echo "<TR><TD id=0>";
$FileZip = 'edesweb_'.date('ymd').'_'.$Tipo;
if( $Tipo == 'x' ){
$Sufijo = 'a';
...
'?'.'>';
$fd = fopen( 'edesweb/m/edes.php', 'w' );
fputs( $fd, $txt );
fclose($fd);
}
uDelFile("edesweb.motor/edesweb/", '/^~/');
$DirExe = '';
if( $Tipo=='z' ){
chdir("edesweb.app");
$di = opendir(".");
...
closedir($di);
chdir("..");
$FileZip = '../edesweb.app/edesweb_'.date('ymd').'_'.$Tipo;
if( strtoupper(substr(PHP_OS,0,3))=='WIN' ) $DirExe = '..\\edesweb\\win\\';
chdir("edesweb.motor");
@unlink("{$FileZip}.zip");
uDelFile("edesweb.motor/", '/^ok_edesweb.app.zip$/');
$ahora = time();
file_put_contents("../edesweb.motor/edesweb/web/aplication/_datos/config/version_edes.cdi", date("Y-m-d H:i:s", $ahora));
eZipDirectory("edesweb", "{$FileZip}.zip");
if( $DirZIP=='edesweb' ){
$nombre_archivo = "{$FileZip}.zip";
if( file_exists($nombre_archivo) ){
$cdi = date("Y-m-d H:i:s", filectime($nombre_archivo));
file_put_contents("../edesweb/web/aplication/_datos/config/version_edes.cdi", $cdi);
touch("{$FileZip}.zip", $ahora);
}
}
echo '  '.eNumberFormat(filesize("{$FileZip}.zip")).' Kb - '.getcwd();
}else{
$FileZip = 'edesweb.BAK/edesweb_'.date('ymd').'_'.$Tipo;
if( strtoupper(substr(PHP_OS,0,3))=='WIN' ) $DirExe = 'edesweb\\win\\';
@unlink($FileZip.'.zip');
eZipDirectory("edesweb", "{$FileZip}.zip");
echo '  '.eNumberFormat(filesize("{$FileZip}.zip")).' Kb';
...
if( $NewDir=='eDes' ){
if( file_exists('edesweb/web/edesweb/_d_/cfg/_ed.lp_') ){
$fd = @fopen('edesweb/web/edesweb/_d_/cfg/_ed.lp_','r');
}else{
exit;
}
}else if( file_exists($__DirOrigen.'edesweb/web/edesweb/_d_/cfg/_ed.lp_') ){
$fd = @fopen($__DirOrigen.'edesweb/web/edesweb/_d_/cfg/_ed.lp_','r');
}else{
exit;
}
...
unlink( 'edesweb/web/edesweb/_d_/cfg/eedd.lp' );
$Buffer = '';
$tmp = explode(chr(10),$txt);
$txt = gzcompress( $txt, 1 );
$Basura = rand(50,250);
...
$nb = ( ceil( strlen($Buffer) / 1959 ) * 1959 ) - strlen($Buffer);
for( $n=0; $n<$nb; $n++ ) $Buffer .= chr(rand(0,255));
$pnt = @fopen('edesweb/web/edesweb/_d_/cfg/eedd.lp','w');
if( !$pnt ) die('');
fputs( $pnt, $Buffer );
fclose( $pnt );
clearstatcache();
touch( 'edesweb/web/edesweb/_d_/cfg/eedd.lp', $_FechaHora );
return true;
}
