...
$NomFile = '/_tmp/pdf/lst_'.$_SESSION['_User'].'.xml';
...
global $_FOOTTITLE;
if( $_FOOTTITLE!='' ){
fputs( $File, utf8_encode("<Row></Row>\n") );
...
fputs( $File, utf8_encode("<Row>\n") );
$Dato = explode("\t",$Dim[$f]);
for( $c=0; $c<count($Dato); $c++ ) xmlText( $File, utf8_encode($Dato[$c]) );
fputs( $File, utf8_encode("</Row>\n") );
}
}
xmlClose( $File );
