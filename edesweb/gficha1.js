...
Obj.children[0].className = 'TABMenuOn';
i = Obj.cellIndex+1;
...
TABMenu.rows[0].cells[a-1].children[0].className = 'TABMenuOn';
TABMenu.rows[0].cells[_nSolapa-1].children[0].className = 'TABMenuOff';
if( !_WinCaption ){
if( S(".WINDOWTITLETEXT").length ){
...
eTabResize();
if( TABMenu.rows[0].cells[sa-1].Func ) eval(TABMenu.rows[0].cells[sa-1].Func)();
SetControl(_nSolapa);
xFRM = "FRM"+_nSolapa;
_eTabOnClick(_nSolapa);
}
...
eval(TABMenu.rows[0].cells[n].ScriptRun)(n+1);
...
var ns =_nOpTab( vNs );
TABMenu.rows[0].cells[ns-1].style.display = (ver) ? 'table-cell' : 'none';
Hojas[ns] = (ver) ? 1 : 0;
