...
S(".TEXTOS INPUT").each(function(k,o){
if( S(o).obj.offsetWidth>0 ) S(o).css("width", o.parentNode.offsetWidth-20);
});
}
...
echo '<td style="background:transparent"><input type="text" value="" id="_Buscar" name="_Buscar" size=25 maxLength255 class=LngTraslate style="color:blue" onkeydown=ControlTeclado() title="" s_tyle="width:100%" onfocus="CntON()" onblur="CntOFF()"></td>';
echo '<td style="background:transparent"><img src=g/buscar.gif onclick="BuscarCadena()"></td>';
echo '</tr></table>';
echo '<script>';
...
echo '<table border=0 cellspacing=0 cellpadding=2 class=TEXTOS style="width:100%">';
echo '<col style="width:20"><col style="width:100%">';
echo '<tr>';
$SinClave = '';
if( $LngBase ){
if( $TieneClave ){
...
echo '<td><input type="text" value="'.$Promp1.'" id="p'.$np.'" name="p'.$np.','.$DimTranslate[$n][3].','.$Tipo.'" size=90 maxLength255 class="LngBase'.$SinClave.'" readonly onfocus="SiguienteCampo()" style="width:100%">';
echo '<tr>';
echo '<td><img src="g/'.$DimImgLeng[$_SESSION['_LANGUAGE_']].'">';
echo '<td><input type="text" value="'.$Promp2.'" ovalue="'.$Promp2.'" id="r'.$np.'" name="r'.$np.','.$DimTranslate[$n][3].'" size=90 maxLength255 class=LngTraslate NLinea='.($DimTranslate[$n][3]+1).(($Tipo=='T')?' readonly style="color:#AAAAAA"':'').' onkeydown=ControlTeclado() title="'.$Ayuda.'" style="width:100%" onfocus="CntON()" onblur="CntOFF()">';
echo '</table>';
}
$np++;
}
...
echo '<td><input type="text" value="'.$Promp1.'" id="lp'.$np.'" name="lp'.$np.','.$DimTranslate[$n][3].','.$Tipo.'" size=90 maxLength255 class="LngBase'.$SinClave.'" readonly onfocus="SiguienteCampo()" style="width:100%">';
echo '<tr>';
echo '<td><img src="g/'.$DimImgLeng[$_SESSION['_LANGUAGE_']].'">';
echo '<td><input type="text" value="'.$Promp2.'" id="lr'.$np.'" name="lr'.$np.','.$DimTranslate[$n][3].'" size=90 maxLength255 class=LngTraslate title="'.$Ayuda.'" style="width:100%" onchange="Repercusion()" onfocus="CntON()" onblur="CntOFF()">';
echo '</table>';
$np++;
}
echo '</table>';
